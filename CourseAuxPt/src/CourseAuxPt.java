...
class CourseAuxPt extends Program{
    ...
    Joueur newJoueur(String nom, int[] pos, int nbStar, int points){ //  Fonction qui crée un objet Joueur
        ...
        j.nbStar=nbStar;
        j.points=points;
        j.inventaire=new Objet[]{listeDesObjets[0]};
        return j;
    }
    ...
    void controleSaisi(Joueur[] joueurs,int idxJ,Case[][] plateau){ //  Contrôle la saisi du joueur et réalise des actions en fonction de la saisie
        ...
        while(!valide){
            ...
            plateauAffichable = plateauToString(plateau, joueurs);
    ...
    void sauvegarde(Joueur[] joueurs,int tour){ //  Sauvegarde les joueurs dans un fichier avec le nom
        ...
        String[][] jeu = new String[length(joueurs)][7];
        int temp;
        for(int i = length(joueurs)-1; i>-1; i--){
            ...
            jeu[i][6] = "|";
            for( int j = 1; j<length(joueurs[i].inventaire); j++){
                jeu[i][6]+= "|" + donneIdxObjet(joueurs[i].inventaire[j]) ;
            }
        }
    ...
    Joueur[] chargerSave(){ //  récupert la sauvegarde des joueurs de la partie précédente
        ...
        for(int i = 0; i<length(joueurs) ;i++){
            ...
            if(length(getCell(save,i,6))>1){
                int[] temp = stringToIdxObjet(getCell(save,i,6));
                for(int j = 0; j < length(temp); j++){
                    ajoutObjet(joueurs[i],listeDesObjets[temp[j]]);
                }
            }
        }
        return joueurs;
    ...
    Effet StringToEffet(String temp){
        ...
        if(equals(temp,"VOLER")){
            ef = Effet.VOLER;
        }else if(equals(temp,"TELEPORTERJ")){
            ef = Effet.TELEPORTERJ;
        }else if(equals(temp,"TELEPORTERJ")){
            ef = Effet.TELEPORTERS;
        }else{
            ef = Effet.NULL;
        }
        return ef;
    ...
    String inventaireToString(Objet[] inv){
        ...
        for(int i = 1; i<length(inv); i++){
            res+=i+")  "+inv[i].icon+"  "+inv[i].nom+"\n";
        }
        return res;
    }
    ...
    void utilisationInventaire(Joueur[] joueurs, int idxUtilisateur,Case[][] plateau){
        ...
        if(length(joueurs[idxUtilisateur].inventaire) > 1 && controleValidation("Voulez-vous utiliser un objet ? (o/n)") ){
            clearScreen();
            println(inventaireToString(joueurs[idxUtilisateur].inventaire));
            int temp = controleSaisi(1, length(joueurs[idxUtilisateur].inventaire),"Quel objet voulez-vous utiliser ?");
    ...
    void gestionInventaire(Joueur[] joueurs, int idxJ,Case[][] plateau){
        for(int i = 1; i < length(joueurs[idxJ].inventaire); i++){
    ...
    void utilisationEffet(Joueur[] joueurs, int idxJ, int indxObj, Case[][] plateau){
        if(joueurs[idxJ].inventaire[indxObj].effet == Effet.VOLER){
            ...
            clearScreen();
        }else if(joueurs[idxJ].inventaire[indxObj].effet == Effet.TELEPORTERJ){
            teleportJoueur(joueurs, plateau);
            clearScreen();
        }else{
            teleportStar(plateau);
            clearScreen();
        }
    }
    ...
    void teleportStar(Case[][] plateau){
        int[] temp = donneIdxStar(plateau);
        modifeIdxStar(plateau, donneIdxStar(plateau));
    }

    boolean verifieMagasin(Case c){
        return equals(c.contenu,"Magasin");
    }
    ...
    void gestionMagasin(Joueur j){
        ...
        String res = "";
        for(int i = 1; i < length(listeDesObjets); i++){
            res += i+") "+ listeDesObjets[i].icon +" "+listeDesObjets[i].nom+ " "+listeDesObjets[i].prix +" Points \n";
        }
        println(res);
    ...
    void actionContenuCase(Joueur j, Case[][] plateau ){
        if(joueurSurStar(donneIdxStar(plateau) ,j.position)){
                verifStar(j, plateau, donneIdxStar(plateau));
                clearScreen();
        }else if(verifieMagasin(donneCaseJoueur(j, plateau))){
                gestionMagasin(j);
                clearScreen();
        }  
    }

    int donneIdxObjet(Objet o){
        int idx = 0;
        while(listeDesObjets[idx].effet!=o.effet){
            idx++;
        }
        return idx;
    }

    int[] stringToIdxObjet(String objets){
        int nbObjet = 0;
        for(int i = 0; i < length(objets) ; i++){
            if(charAt(objets,i) == '|'){
                nbObjet++;
        ...
        int[] truc = new int[nbObjet];
        int idx = 0;
        for(int j = 0; j < length(objets) ; j++){
            if(charAt(objets,j) != '|'){
                truc[idx] = stringToInt(substring(objets,j,j+1));
                idx++;
            }
        }
        return truc;
    ...
    String chargerAscii(String filename){
        CSVFile art = loadCSV(filename);
        String res = "";
        for(int i = 0; i < rowCount(art); i++){
            res += getCell(art,i,0) + "\n";
        }
        return res; 
    }
    ...
    void ecranTitre(){
        clearScreen();
        println(chargerAscii("ressources/titre.csv")+"\n\n\n\n\n\n\n\n\n\n\n\n");
        println("                                                      Appuyer sur ENTRER pour COMMENCER");
        readString();
        clearScreen();
        println("             Dans ce jeu vous affronterez vos amis ! Pour gagner vous devrez acheter le plus d'étoiles avec vos points.\n             Points qui seront gagner en répondant à des questions. Et plus vite vous répondez plus vous gagnez des points ! \n\n\n\n\n\n\n\n\n\n\n\n");
        println("                                                      Appuyer sur ENTRER pour PASSER");
        readString();
        clearScreen();
    }
    ...
    void algorithm(){
        ...
        int idxTourJoueur = 0;
        clearScreen();
        int nbTours = nbToursPartie()*length(joueurs);
        while(nbTours > 0){
            String plateauAffichable = plateauToString(plateau,joueurs);
